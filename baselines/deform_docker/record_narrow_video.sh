#!/bin/bash
# Record narrow-approach DEFORM demo to MP4 via rgb_array (no X11/ffmpeg).
# Usage: ./record_narrow_video.sh [output_basename]
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BASENAME="${1:-deform_4agents_near_narrow_manual}"
...
docker run --rm \
  ...
  bash -lc "source /opt/ros/noetic/setup.bash && source /root/DEFORM/devel/setup.bash && \
    roslaunch f1tenth_deform_bridge deform_f1tenth_4agents_narrow_render_upstream.launch \
