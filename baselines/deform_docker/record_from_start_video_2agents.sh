#!/bin/bash
# Record 2-agent from-track-start DEFORM demo (rgb_array, no X11).
# Usage: ./record_from_start_video_2agents.sh [output_basename]
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BASENAME="${1:-deform_2agents_from_start}"
OUT="${SCRIPT_DIR}/results/videos/${BASENAME}.mp4"
mkdir -p "${SCRIPT_DIR}/results/videos"

echo "[record 2-agent from start] → ${OUT}"
docker run --rm \
  ...
  --volume "${SCRIPT_DIR}/f1tenth_deform_bridge/config/bridge_params_full_narrow_2agents.yaml":/root/DEFORM/src/utility/f1tenth_deform_bridge/config/bridge_params.yaml:ro \
  --volume "${SCRIPT_DIR}/f1tenth_deform_bridge/config/frm_shape_2robots.yaml":/root/DEFORM/src/plan_manager/config/frm_shape.yaml:ro \
  ...
  bash -lc "source /opt/ros/noetic/setup.bash && source /root/DEFORM/devel/setup.bash && \
    roslaunch f1tenth_deform_bridge deform_f1tenth_2agents_from_start_render.launch \
