...
# ── Narrow-zone metrics (optional — requires eval_metrics.py in baselines/) ──
_BASELINES_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
if _BASELINES_DIR not in sys.path:
    sys.path.insert(0, _BASELINES_DIR)
try:
    from eval_metrics import NarrowZoneMetrics as _NarrowZoneMetrics
except ImportError:
    _NarrowZoneMetrics = None
...
def collect_stats(
    ...
    # ── Resolve track centerline once (reused across episodes) ───────────
    _raw_env = getattr(env, "unwrapped", env)
    _track   = getattr(_raw_env, "track", None)
    ...
    for i in range(n_episodes):
        ...
        # ── Fresh zone collector for this episode ─────────────────────────
        zone = None
        if _NarrowZoneMetrics is not None and _track is not None:
            zone = _NarrowZoneMetrics(
                centerline_xs=_track.centerline.xs,
                centerline_ys=_track.centerline.ys,
                dt=0.1,   # CBF env timestep
            )
        # ─────────────────────────────────────────────────────────────────
        ...
        while not done:
            ...
            if zone is not None:
                _state = env.state
                _ids   = list(_state.keys())
                zone.step({
                    "poses_x":       [_state[a]["pose"][0]     for a in _ids],
                    "poses_y":       [_state[a]["pose"][1]     for a in _ids],
                    "linear_vels_x": [_state[a]["velocity"][0] for a in _ids],
                })
            # ─────────────────────────────────────────────────────────────
        ...
        # ── Merge zone metrics into episode stats ─────────────────────────
        if zone is not None:
            for k, v in zone.summary().items():
                ep_statistics[f"zone_{k}"] = v
        # ─────────────────────────────────────────────────────────────────
